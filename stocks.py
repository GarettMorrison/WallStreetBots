...
with open("StockTickers.txt","r") as inputFile:
...
def checkString(inString, bucket):
	...
	for i in range(tickerCount):
		if tickerArray[i][0] in inString:
			# if checkString in inString:
			print(tickerArray[i][0])
				# print(bucket)
...
def checkMentions(subredditName, iterative):
	...
	maxTime = 0
	timeSet = 0
	for submission in reddit.subreddit(subredditName).search("What Are Your Moves Tomorrow,",limit = None):
		iterative += 1
		if iterative > 50:
			saveState()
			iterative = 0
...
print("WSB TIME _________________________________________")
